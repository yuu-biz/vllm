...
control_vector_path = "raywanb/mistral-cv-example/mistral-7b-v0.1-control-vector.gguf"
...
def do_sample(engine):
    ...
    prompts = [(prompt_text,
                SamplingParams(temperature=0.0,
                               max_tokens=100,
                               stop=["[/assistant]"]),
                ControlVectorRequest("chaotic", 1, control_vector_path, 1.0)),
               (prompt_text,
                SamplingParams(temperature=0.0,
                               max_tokens=100,
                               stop=["[/assistant]"]), None)]
...
def test_control_vector_adapter():
