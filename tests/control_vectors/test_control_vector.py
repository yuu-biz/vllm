...
def test_offline_inferance(requests, enforce_eager):
    ...
    for request in requests:
        ...
        result = llm.generate(
            prompt, sampling_param, control_vector_request=control_vector_request
        )
        results.append(
            {
                "prompt": prompt,
                "generation": result[0].outputs[0].text,
                "control_vector_name": control_vector_request.control_vector_name
                if control_vector_request
                else None,
                "scale": control_vector_request.scale_factor
                if control_vector_request
                else None,
            }
        )
